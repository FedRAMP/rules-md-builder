...
This repository tracks the `pwx-buildout2` branch of `FedRAMP/rules` as a git submodule at `rules` so the source material stays owned upstream instead of being edited here.

The `rules` submodule is configured with sparse checkout so only `fedramp-consolidated-rules.json` and `schemas/fedramp-consolidated-rules.schema.json` are materialized in the working tree. Git still keeps submodule metadata under `.git/modules`, but the checked-out filesystem stays focused on the upstream files this repository needs.
